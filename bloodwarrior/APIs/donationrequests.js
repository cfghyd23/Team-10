...
const donaterequestApp=exp.Router()
const jwttoken=require("jsonwebtoken")
const authUser=require("../middleware/authUser")
donaterequestApp.use(exp.json())
...
donaterequestApp.get('/userprofile',authUser,async(request,response)=>{
    // let id=(+request.params.id)
    // let userobj=users.find(userobj=>userobj.id==id)
    // response.send({message:"user found",payload: userobj});
    let User=request.app.get("donationrequestsObj")
...
donaterequestApp.post('/getrequests',async(request,response,next)=>{
    try{
        //console.log(request)
        //user collection object
        let usersObj=request.app.get("donationrequestsObj")
        let usercred=request.body;
            ...
            response.send({message:"user doesn't exists"});
             ...
             if(status==false)
             {
                response.send({message:"Invalid password"});
                 ...
                 response.send({message:"login successful",userobj:finduser});
             }
        }
    }
    catch(err){
...
donaterequestApp.post('/createrequests',async(request,response,next)=>{
    try{
        //user collection object
        let usersObj=request.app.get("donationrequestsObj")
...
module.exports=donaterequestApp;